...
# 训练集文件
train_images_idx3_ubyte_file = 'D:/作业存档/Lenet-5 2019-1-16/mnist/train-images.idx3-ubyte'
# 训练集标签文件
train_labels_idx1_ubyte_file = 'D:/作业存档/Lenet-5 2019-1-16/mnist/train-labels.idx1-ubyte'

# 测试集文件
test_images_idx3_ubyte_file = 'D:/作业存档/Lenet-5 2019-1-16/mnist/t10k-images.idx3-ubyte'
# 测试集标签文件
test_labels_idx1_ubyte_file = 'D:/作业存档/Lenet-5 2019-1-16/mnist/t10k-labels.idx1-ubyte'
...
def lenet5():
    ...
    bitchSize = 200  # 每bitchSize张图片更新一次参数
    # M = 0
...
#            feature_map3_0[15] = feature_map3_0 / 6
        ...
        exper = exper + 1  # 计算次数
        print('次数', exper)
        print('----------------5-19-200-He-BGD-even-rot-test----------------------')
    ...
    for i in range(bitchSize):
        ...
        for k in range(0, 6):
            feature_map1[k] = convolution(
                test_image, conv1[k]) + conv1_b[k]  # 输入图片分别与六个卷积核相关
        feature_map1_relu = relu(feature_map1)  # 激活函数
        '''
        池化层P2
        pool:池化函数，图片大小减半添加偏置
