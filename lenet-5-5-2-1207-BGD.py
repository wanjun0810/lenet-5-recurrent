...
def lenet5():
    ...
    while (exper < 10000):
        ...
        for i in range(bitchSize):
            ...
            output_max = np.max(output)  # 预防softmax上溢出和下溢出算法需要
        ...
        # 反向传播
            ...
            # output_w_loss = Loss对output层的偏导 * np.sum(hide)
            ...
            for j in range(120):
                sigma_pool4_1[j] = convolution(
                    sigma_pool4_0[j], np.rot90(conv5[j], 2))  # 计算该层灵敏度的一部分
            ...
            # 某节点对应的偏导的和即在计算下一层结点时用到了该结点的所有的结点反向传回的偏导的和 即sigma_pool2_2
            ...
            sigma_conv1 6*28*28
            '''
            sigma_conv1_0 = np.zeros((6, 28, 28))
            for j in range(6):
        ...
        exper = exper + 1  # 计算次数
        print('次数', exper)
        print('----------------5-2-train-He-BGD-even-rot-----------------------')
    ...
    for i in range(bitchSize):
        ...
        for j in range(10):
            if softmax_result[j] == softmax_result_max:
                ans = j
        print('学习结果：', ans, '正确结果:', train_labels[i])
